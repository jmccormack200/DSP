...
audio_chR = ((audio_IN >>16)& 0x0000FFFF); //Separate 16 bits channel right
 ...
 input =(float32_t)(audio_chL);  // Compute only the left channel
...
audio_OUT = ((audio_out_chL<<16 & 0xFFFF0000)) + (audio_out_chL & 0x0000FFFF);	//Put the two channels toguether again
i2s_tx(audio_OUT);
